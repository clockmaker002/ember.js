...
function register(renderer) {
  assert('renderer is not registered', renderers.indexOf(renderer) === -1);
  renderers.push(renderer);
}

function deregister(renderer) {
  let index = renderers.indexOf(renderer);
  assert('renderer is registered', index !== -1);
...
class Renderer {
  ...
  _renderRoot(root, template, self, target, dynamicScope) {
    assert(!this._root, 'root is not in DOM');
    this._root = root;
    register(this);
